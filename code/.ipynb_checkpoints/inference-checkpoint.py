...
def predict_fn(input_data, model):
    ...
    try:
        inference_fn = model.signatures["serving_default"]
        predictions = inference_fn(**input_tensors)
        relevance_scores = predictions["output_0"].numpy()
    except Exception as e:
        raise ValueError(f"Inference failed: {e}")

    
    #predictions = model(input_tensors)["output_0"].numpy()
    #relevance_scores = predictions['output_0'].numpy()
    # Add predictions to titles dataframe
    titles_df["relevance_score"] = relevance_scores.flatten()
